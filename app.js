 ...
 * Master Client Application Script (Updated & Event-Bound)
 */
...
(function () {
  ...
  function formatMoney(amountInUSD) {
    ...
    if (currentCurrency === 'JPY') return `${curr.symbol}${converted.toLocaleString()}`;
    if (currentCurrency === 'SAR' || currentCurrency === 'AED') return `${converted.toLocaleString()} ${curr.symbol}`;
  ...
  // ==========================================================================
  // 6. Day Details & Modal Rendering Logic
  // ==========================================================================
  function openDayDetailsModal(dayIndex) {
    const trip = getCurrentTrip();
    if (!trip || !trip.days || !trip.days[dayIndex]) return;

    const day = trip.days[dayIndex];
    const modal = document.getElementById('dayDetailsModal');
    const titleEl = document.getElementById('modalDayTitle');
    const subtitleEl = document.getElementById('modalDaySubtitle');
    const bodyEl = document.getElementById('modalDayDetailsBody');

    if (!modal || !bodyEl) return;

    if (titleEl) titleEl.textContent = `Day ${day.dayNumber}: ${day.neighborhood}`;
    if (subtitleEl) subtitleEl.textContent = day.weatherPlan || 'Daily Period Breakdown';

    bodyEl.innerHTML = '';

    const slots = [
      { phase: 'morning', phaseName: t('slotMorning') || 'Morning', slotData: day.morning },
      { phase: 'lunch', phaseName: t('slotLunch') || 'Lunch', slotData: day.lunch },
      { phase: 'afternoon', phaseName: t('slotAfternoon') || 'Afternoon', slotData: day.afternoon },
      { phase: 'evening', phaseName: t('slotEvening') || 'Evening', slotData: day.evening }
    ];
    ...
    slots.forEach(s => {
      ...
      card.innerHTML = `
        <div class="slot-time-column">
          ...
          <span class="slot-time-range">${s.slotData.time || ''}</span>
        </div>
  ...
  function bindDayCardClickEvents() {
    const dayCards = document.querySelectorAll('.day-pill-btn, .day-card, .day-details-trigger');
    dayCards.forEach((card) => {
      card.addEventListener('click', (e) => {
        const dayIdxAttr = card.getAttribute('data-day-index');
        const dayIdx = dayIdxAttr !== null ? parseInt(dayIdxAttr, 10) : activeDayIndex;
        
        // Open modal when triggering cards or direct modal triggers
        if (card.classList.contains('day-card') || card.classList.contains('day-details-trigger')) {
          openDayDetailsModal(dayIdx);
        }
      });
    });

    const btnClose = document.getElementById('btnCloseDayModal');
    const btnSecondaryClose = document.getElementById('btnSecondaryCloseDayModal');
    const modal = document.getElementById('dayDetailsModal');

    if (btnClose) btnClose.onclick = closeDayDetailsModal;
    if (btnSecondaryClose) btnSecondaryClose.onclick = closeDayDetailsModal;
  ...
  function renderItinerary() {
    ...
    let slotsHtml = phases.map(phase => {
      ...
      if (!slot) return '';
      return `
        <div class="timeline-slot-card day-card ${slot.completed ? 'completed' : ''}" data-day-index="${activeDayIndex}">
          <div class="slot-time-column">
          ...
          <div class="slot-content-column">
            ...
            <div class="slot-actions-bar">
              <label class="completion-check-label" onclick="event.stopPropagation();">
              ...
              <div class="slot-btn-group" onclick="event.stopPropagation();">
    ...
    container.innerHTML = `
      <div class="itinerary-header-bar" style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap; margin-bottom: 1rem;">
        <div class="day-pills-scroller">${dayPillsHtml}</div>
        <button class="btn btn-sm btn-secondary day-details-trigger" data-day-index="${activeDayIndex}" type="button">🔍 View Full Day Modal</button>
      </div>
